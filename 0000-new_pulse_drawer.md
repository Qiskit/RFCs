...
However when we enable conditional gates or a conditional reset in the middle in the schedule, the truncation by `plot_range` doen't work becase users may want to know the pulse sequence after the measurement stimulus.
There is another frustrating situation based on our experience. When we wrote the Qiskit Pulse paper we needed to modify the drawer itself to adress reviewers requests and we decided not to share the code for pulse drawing in the shared database.
