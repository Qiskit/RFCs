...
**A.** Remove the abstract `run` method from `QuantumAlgorithm`, let every algorithm decide how the main
execution should be called.

**B.** Keep `run` but let every algorithm define a more specific alias.

**C.** Keep `run` for some algorithms, but let other classes of algorithms define a more specific interface without a `.run`.
...
1. `AmplitudeEstimationAlgorithm` (already exists) - TODO 
1. `MinEigensolver` - Many algorithms require a MinEigensolver as a subroutine, and an enforced interface would allow users to pass their eigensolver of choice into an application or meta-algorithm.
    1. Instance methods may include:
        1. `get_min_eigenvalue` - As described above, should return the result if the result has already
    1. Examples: VQE, QAOA, QPE, ExactEigensolver 
1. `Classifier` - TODO 
    1. Examples: VQC, QSVM, Classical SVM
1. `LinearSolver`- TODO
    1. Examples: HHL, ExactLSsolver
1. `ExpectationAlgorithm` - TODO
1. `EvolutionAlgorithm` - TODO
1. `VariationalOptimizer` - TODO
1. `GradientAlgorithm` - TODO
...
**Decision** The present organization of the algorithm directory is unintuitive, and relies on naming conventions which are not industry standard in Quantum algorithms. We should update this to reflect a more intuitive structure. Some options:
